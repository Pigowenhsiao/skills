...
這份文件是 `10-LLM-Wiki` 的 reference playbook，用來說明 YouTube 來源如何整理進 `PigoVault`，以及何時再同步到 Notion。
...
在目前 vault 中，YouTube 新來源的預設 capture 位置是：

- `00-Inbox/`

整理完成後，再依內容性質移入：

- `09-Article-Notes/<kind>/<topic>/`：已整理成 article note、source note、guide、tool、case、comparison 等
- `09-Article-Notes/99-transcripts/`：需要保留逐字稿或 transcript 脈絡
- `08-Learning/<topic-first 子目錄>/`：學習型主題知識
- `08-Learning/90_Source-Inbox/`：未萃取但需保留的來源材料
...
若內容已整理完成，可在 Pigo 明示或整理流程中改放：

- `09-Article-Notes/<kind>/<topic>/`
- `09-Article-Notes/99-transcripts/`
- `08-Learning/<topic-first 子目錄>/`
...
YouTube 整理流程的重點不是把影片硬轉成長文，而是先把可驗證的內容落到 `00-Inbox`，再依整理結果移入正確 vault 類別，必要時才同步到 Notion。
