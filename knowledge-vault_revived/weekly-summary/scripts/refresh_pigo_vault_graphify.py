...
def patch_graphify_networkx_compat() -> None:
    """Bridge graphify's older `edges=` call to networkx 3.3's `link=` parameter."""
    if "edges" in inspect.signature(nx_json_graph.node_link_data).parameters:
        return

    original = nx_json_graph.node_link_data

    def compat_node_link_data(G, *args, **kwargs):
        if "edges" in kwargs and "link" not in kwargs:
            kwargs["link"] = kwargs.pop("edges")
        return original(G, *args, **kwargs)

    nx_json_graph.node_link_data = compat_node_link_data


patch_graphify_networkx_compat()
