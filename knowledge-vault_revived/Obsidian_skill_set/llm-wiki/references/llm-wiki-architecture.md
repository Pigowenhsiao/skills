...
5. **Lint 不只是抓錯，還要找成長方向** — 健康檢查除了 broken links、孤兒頁、過時內容，也應輸出知識缺口、應建立的核心頁面、應補查的新來源。

6. **把控制平面分開** — `AGENTS.md` 管 agent 行為與本地工作規則，`SCHEMA.md` 管 wiki 結構、分類與 frontmatter。兩者都要讀，衝突時以使用者指令與 `AGENTS.md` 為先。
