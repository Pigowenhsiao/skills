...
1. URL / article / X / YouTube:
   - prefer `markitdown`
   - use `/Users/pigo/Documents/Agent/markitdown/scripts/run_markitdown.sh`
   - if direct capture fails or is obviously incomplete, fall back to `baoyu-url-to-markdown`
   - if the source is YouTube and `markitdown` does not yield a usable transcript/body, fall back to `baoyu-youtube-transcript`
2. PDF / Office / local files:
   - prefer `markitdown`
3. Already-available Markdown or pasted text:
   - use it directly without re-fetching

Treat these as fallback signals for URL sources:

- empty or near-empty output
- only navigation / login wall / cookie wall
- dynamic page shell without article body
- X thread missing main content
- YouTube link missing transcript or meaningful page text
