...
The `claude-permissions-optimizer` skill is for optimizing Claude Code permissions by finding safe Bash commands from session history and auto-applying them to settings.json. Can run from any coding agent but targets Claude Code specifically.

Optimize Claude Code permissions by finding safe Bash commands from session history and auto-applying them to settings.json. Can run from any coding agent but targets Claude Code specifically. Use it when experiencing permission fatigue, too many permission prompts, wanting to optimize permissions, or needing to set up allowlists. Triggers on "optimize permissions", "reduce permission prompts", "allowlist commands", "too many permission prompts", "permission fatigue", "permission setup", or complaints about clicking approve too often.
...
Then proceed to Step 1 normally. The skill works from any environment as long as `~/.codex/` (or `$CLAUDE_CONFIG_DIR`) exists on the machine.
...
1. **Apply all to user settings** (`~/.codex/settings.json`)
2. **Apply all to project settings** (`.codex/settings.json`)
3. **Skip**
...
```
Applied N rules to ~/.codex/settings.json
Applied M rules to .codex/settings.json

These commands will no longer trigger permission prompts.
```

If `.codex/settings.json` was modified and is tracked by git, mention that committing it would benefit teammates.

## Edge Cases

- **No project context** (running outside a project): Only offer user-level settings as write target.
- **Settings file doesn't exist**: Create it with `{ "permissions": { "allow": [] } }`. For `.codex/settings.json`, also create the `.codex/` directory if needed.
