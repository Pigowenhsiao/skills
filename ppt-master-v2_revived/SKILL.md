...
description: >
  ...
  multi-role collaboration. Use when the user asks to "create PPT", "make presentation",
  "make slides", "製作簡報", "做簡報", "幫我做簡報", "產生簡報",
  "製作投影片", "把文件轉成簡報", "把文章整理成簡報", "把 PDF 轉成簡報",
  "匯出成 PPTX", or mentions "ppt-master".
triggers:
  - "ppt-master_v2"
  - "做簡報"
  - "製作簡報"
...
| Failure | Handling |
|---|---|
| CDP connection timeout / daemon down | run `bb-browser restart` or `bb-browser start`, verify port `19825` is open and retry |
| AI image generation fails / API key error | do NOT halt pipeline; retry once, mark image row as `Needs-Manual`, report filename + prompt to user, and proceed to Step 6 |
| SVG validation error (from quality checker) | locate failed page, return to Step 6, regenerate SVG, and ensure all tags are closed and follow shared-standards.md |
| Non-system/unsupported font warning | fall back to system safe fonts (e.g., Arial, Inter, Roboto, sans-serif) to prevent PowerPoint rendering artifacts |
| Chart coordinates misaligned | run the standalone `verify-charts` workflow to manually calibrate pixel offsets |
| total_md_split.py / finalize_svg.py errors | verify directory structure exists, check file permissions, and run commands individually in sequence |
| svg_to_pptx.py export fails | check python-pptx dependency, ensure no locked instance of the PPTX file is open, and verify final SVG files exist in `svg_final/` |
...
- Canvas format and page count
- Finalized SVG files path (`svg_final/`)
- Speaker notes path (`notes/`)
- Exported PPTX file path (`exports/<project_name>_<timestamp>.pptx`)
- svg_quality_checker.py validation status (Errors: 0)
- Applied animation and transition options
