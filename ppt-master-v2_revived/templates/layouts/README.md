...
# Directory: skills\ppt-master\templates\layouts
...
- **Full Index**: [README.md](./README.md) (human browsing â€” includes categories, primary colors, detailed tone)
- **Slim Index**: [layouts_index.json](./layouts_index.json) (lightweight lookup â€” `label` / `summary` / `keywords` only)
...
| `china_telecom_template` | Brand | Telecom solutions, digital transformation plans,æ”¿ä¼æ±‡æŠ¥ | Telecom Red `#C00000` | Restrained, authoritative, telecom enterprise style |
| `ä¸­æ±½ç ”_å¸¸è§„` | Brand | Product certification, evaluation & testing | Deep Blue `#004098` | [Standard] Professional authority, consulting style |
| `ä¸­æ±½ç ”_å•†åŠ¡` | Brand | Business visits, technical exchanges | Blue Gradient `#003366` | [Business] Modern tech, composed and sophisticated |
| `ä¸­æ±½ç ”_çŽ°ä»£` | Brand | Strategic launches, future tech | Deep Blue `#001529` | [Future] Future Tech, neon glow |
| `ä¸­å›½ç”µå»º_å¸¸è§„` | Brand | Power & energy, engineering, state-owned enterprise reports | PowerChina Blue `#00418D` | Craftsmanship, steady and reliable |
| `ä¸­å›½ç”µå»º_çŽ°ä»£` | Brand | International engineering, premium roadshows, tech innovation | Deep Sea Blue `#001F45` | [Modern] Grand narrative, digital tech |
| `æ‹›å•†é“¶è¡Œ` | Brand | Premium reports, VIP services, annual reports | CMB Red `#C41230` | Minimalist luxury, financial texture, borderless |
...
| `ç§‘æŠ€è“å•†åŠ¡` | General | Corporate reports, product launches, proposals | Tech Blue `#0078D7` | Tech, business, professional, clean |
| `smart_red` | General | Tech company profiles, education solutions | Smart Red-Orange `#DE3545` | Modern, vibrant, geometric |
| `é‡åº†å¤§å­¦` | Scenario | Academic defense, research presentations | CQU Blue `#006BB7` | Academic solidity, mountain-city character |
---
...
| `ä¸­æ±½ç ”_å¸¸è§„` | CATARC standard style (v1), suitable for certification and evaluation |
| `ä¸­æ±½ç ”_å•†åŠ¡` | CATARC business style (v2), modern tech business, composed and sophisticated |
| `ä¸­æ±½ç ”_çŽ°ä»£` | CATARC modern style (v3 Future), Future Tech style, deep blue + neon cyan |
| `ä¸­å›½ç”µå»º_å¸¸è§„` | PowerChina standard style (v1), suitable for power, energy, and engineering SOEs |
| `ä¸­å›½ç”µå»º_çŽ°ä»£` | PowerChina modern style (v2), emphasis on grand narrative and digital tech |
| `æ‹›å•†é“¶è¡Œ` | China Merchants Bank v2.0, minimalist luxury, borderless open layout |
...
| `ç§‘æŠ€è“å•†åŠ¡` | Tech business style, rigorous and professional, hexagonal texture |
...
| `é‡åº†å¤§å­¦` | Chongqing University template, blending mountain-city layered imagery with modern academic style |
...
|----------------|-------------|
| HTML named entities in text (`&nbsp;` `&mdash;` `&copy;` `&ndash;` `&reg;` â€¦) | Write the raw Unicode character directly (`â€”` `â€“` `Â©` `Â®` `â†’` NBSP â€¦); see shared-standards.md Â§1.0 |
| Bare `&` `<` `>` `"` `'` in text or attribute values | Escape as XML entities `&amp;` `&lt;` `&gt;` `&quot;` `&apos;` (e.g. `R&amp;D`, `error &lt; 5%`) |
| `<foreignObject>` | Use `<text>` + `<tspan>` |
| `clipPath` on shapes / groups / text | Draw the target geometry directly with the matching native element (`<circle>` / `<ellipse>` / `<rect rx>` / `<polygon>` / `<path>`). `clipPath` on `<image>` elements is conditionally allowed â€” see shared-standards.md Â§1.2 |
