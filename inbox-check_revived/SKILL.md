...
name: inbox-check
description: >
  Use when multiple notes in 00-Inbox should be processed in one batch: scan,
  classify by content, route to correct vault location, update MOCs, extract action
  items, and generate a daily digest. This skill owns batch inbox scanning,
  safe routing, and deferred-item reporting, not single-note deep formalization. Triggers:
...
# Inbox Check â€” Intelligent Inbox Processing & Filing
...
- Use it when the user means "整理整個 inbox", "整理 00-Inbox", "整理 inbox", "清 inbox", or "triage the inbox".
- It should safely classify, route, defer, and report across many notes.
- It may move low-risk notes, leave ambiguous notes in place, and update nearby navigation files.
- It should not deeply rewrite every note into a polished knowledge article during the triage pass.

`note-update` owns one-note-at-a-time formalization.

- If a note is safely classifiable but clearly needs deeper rewriting, restructuring, or editorial cleanup, surface it as follow-up guidance instead of mixing that work into the batch triage.
- Recommended follow-up from this skill is advisory only. The dispatcher does not auto-run another skill just because this section appears.

Use this advisory format when a note should be handed off for deeper single-note work:
...
If the follow-up is another skill rather than another agent, do not misuse `### Suggested next agent`. Use `### Recommended follow-up skill` as advisory guidance for the user/dispatcher instead.
