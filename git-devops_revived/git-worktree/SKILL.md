...
The `git-worktree` skill covers this skill manages Git worktrees for isolated parallel development. It handles creating, listing, switching, and cleaning up worktrees with a simple interactive interface, following KISS principles.
...
1. **Code Review (`/prompts:ce-review`)**: If NOT already on the target branch (PR branch or requested branch), offer worktree for isolated review
2. **Feature Work (`/prompts:ce-work`)**: Always ask if user wants parallel worktree or live branch work
...
The skill is automatically called from `/prompts:ce-review` and `/prompts:ce-work` commands:
