...
The `rclone` skill covers upload, sync, and manage files across cloud storage providers using rclone.

Upload, sync, and manage files across cloud storage providers using rclone. Use it when uploading files (images, videos, documents) to S3, Cloudflare R2, Backblaze B2, Google Drive, Dropbox, or any S3-compatible storage. Triggers on "upload to S3", "sync to cloud", "rclone", "backup files", "upload video/image to bucket", or requests to transfer files to remote storage.
...
curl https:/the rclone skill.org/install.sh | sudo bash
